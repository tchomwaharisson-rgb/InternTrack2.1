 ...
 */
function getRegistrationApprovalEmail($userName, $email, $password = null, $role = 'intern') {
    $app_name = SITE_NAME;
    ...
    <head>
        ...
        <style>
            ...
            .header { background: linear-gradient(135deg, #D32F2F 0%, #B71C1C 100%); padding: 30px 40px; text-align: center; color: white; }
            .header h1 { margin: 0; font-size: 28px; font-weight: 800; }
            .header p { margin: 8px 0 0; opacity: 0.9; font-size: 14px; }
            .content { padding: 40px; }
            .content h2 { color: #1f2937; margin-top: 0; font-size: 22px; }
            .content p { color: #4b5563; margin: 0 0 16px; }
            .success-icon { text-align: center; font-size: 64px; margin: 16px 0; }
            .info-box { background: #fef2f2; border-left: 4px solid #16a34a; padding: 16px 20px; border-radius: 6px; margin: 16px 0; }
            .info-box strong { color: #16a34a; }
            .button { display: inline-block; background: linear-gradient(135deg, #D32F2F 0%, #B71C1C 100%); color: white; padding: 14px 32px; text-decoration: none; border-radius: 8px; font-weight: 600; margin: 16px 0; }
            .button:hover { transform: translateY(-2px); box-shadow: 0 4px 15px rgba(211, 47, 47, 0.3); }
            ...
            .credentials { background: #f9fafb; border-radius: 8px; padding: 16px 20px; margin: 16px 0; border: 1px solid #e5e7eb; }
            .credentials code { display: block; background: white; padding: 8px 12px; border-radius: 4px; border: 1px solid #e5e7eb; font-family: monospace; font-size: 14px; margin: 4px 0; }
            @media (max-width: 480px) { .header { padding: 20px; } .content { padding: 24px; } .footer { padding: 16px 20px; } }
        </style>
    </head>
    <body>
        <div class="container">
    ...
    if ($password) {
        $html .= '
                ...
                <div class="credentials">
                    <code style="font-size: 16px; font-weight: 600; color: #D32F2F;">' . htmlspecialchars($password) . '</code>
                </div>
