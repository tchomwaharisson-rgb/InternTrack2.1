    ...
    switch ($action) {
        case 'approve':
            ...
            if ($request) {
                ...
                if ($existing_user) {
                    // Email already exists - reject this request with error
                ...
                if ($duplicate_request) {
                    // Another pending request exists - reject this one
                    $stmt = $conn->prepare("UPDATE registration_requests SET status = 'rejected', 
                                           reviewed_at = NOW(), reviewed_by = ?, admin_comment = ? WHERE id = ?");
                ...
                // USE THE USER'S ORIGINAL PASSWORD (already hashed in the request)
                ...
                try {
                    $stmt->execute([
                        $request['email'],
                        $password_hash, // Use the original hashed password
                        $request['first_name'],
                        $request['last_name'],
                        $request['role']
                    ]);
                } catch (PDOException $e) {
                    // If insertion fails, update request status
                    $stmt = $conn->prepare("UPDATE registration_requests SET status = 'rejected', 
                                           reviewed_at = NOW(), reviewed_by = ?, admin_comment = ? WHERE id = ?");
